...
var userStore = __webpack_require__(2);
var gameStore = __webpack_require__(1);
...
var showGame = function showGame(sz) {
...
var updateGame = function updateGame(jsonObj) {
    return $.ajax({
        url: config.apiUrl + '/games/' + gameStore.game.id,
        method: 'PATCH',
        headers: {
...
var apiUrl = void 0;
var apiUrls = {
  // production: 'https://tic-tac-toe-js.herokuapp.com/',
...
var board = __webpack_require__(18);

var winners = []; // array to hold winning combinations
var SIZE = 3;
var playerTokenX = 'X';
var playerTokenY = 'O';
var currentPlayer = 0;
...
var player1 = new Player.Player('player1', 0);
var player2 = new Player.Player('player2', 1);
...
var handler = function handler(e) {
    ...
    move++;
    var isWin = checkWinner(player1, player2, currentPlayer);
...
var checkWinner = function checkWinner(player1, player2, currentPlayer) {
    // win condition
    var isWin = false;
    ...
    if (playerSelections.length >= SIZE) {
        for (var winningSet = 0; winningSet < winners.length; winningSet++) {
            var sets = winners[winningSet]; // winning hand
            var setFound = true;
...
var getFormFields = __webpack_require__(11); // supplied module from GA
...
var onGetGame = function onGetGame() {
    ...
    gameApi.showGame().then(function () {
        var parent = document.getElementById('old-games');
...
var onGetGames = function onGetGames() {
    gameApi.indexGames().then(function (data) {
        var parent = document.getElementById('old-games');
        while (parent.hasChildNodes()) {
            parent.removeChild(parent.firstChild);
        }
...
var Player = function Player(name, gameId) {
...
var updateEvent = __webpack_require__(19);
var gameEvents = __webpack_require__(20);

var updateBoard = function updateBoard(cellClicked) {
    var updateGameObj = {
        'game': {
            'cell': {
    ...
    var cellIndex = cellClicked.dataset.index;
...
var createScoreBoard = function createScoreBoard() {
    ...
    var player1ScoreHeading = document.createElement('p');
    var player2ScoreHeading = document.createElement('p');
    var playerOneScoreValue = document.createElement('div');
    var playerTwoScoreValue = document.createElement('div');
...
var reset = function reset(player1, player2, currentPlayer) {
    currentPlayer = 0;
...
var onUpdateGame = function onUpdateGame(gameObj) {
    gameApi.updateGame(gameObj).then(function (data) {
...
var onCreatNeweGame = function onCreatNeweGame() {
    // event.preventDefault()
    gameApi.createGame().then(function (data) {
        gameStore.game = data.game;
        $('#newGame').toggleClass('hidden');
...
module.exports = {
    onCreatNeweGame: onCreatNeweGame
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
